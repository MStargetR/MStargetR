...
- **R** >= 4.1.0. Download and install from <https://cran.r-project.org/>.
...
- **Bioconductor dependencies** -- the packages `mzR`, `ropls`, and `statTarget`
  ...
  `sva` is additionally required for ComBat correction (`batch_method = "ComBat"`
  in `qcCheckR()`, or `method = "ComBat"` in `batchCorrectR()`). It is only a
  suggested dependency, so install it separately with

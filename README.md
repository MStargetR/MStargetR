...
# MStargetR <img src='man/figures/logo.png' align="right" height="139" />

[![license](https://img.shields.io/github/license/MStargetR/MStargetR.svg?color=%23A31F34)](LICENSE)
...
<a href="https://raw.githubusercontent.com/MStargetR/MStargetR/main/man/figures/architecture.png" target="_blank" title="Click to enlarge" class="zoomable-figure"><img src="man/figures/architecture.png" alt="MStargetR pipeline architecture: vendor raw files pass through msConvertR, PeakForgeR and qcCheckR to produce HTML, Excel and .qs2 outputs" width="100%" /></a>
...
- **Vendor file conversion** – `msConvertR()` converts proprietary
  instrument files (SCIEX .wiff, Bruker .d, Thermo .raw, Waters .raw,
  and others) to open mzML format using ProteoWizard msConvert through
  Docker.
- **Peak picking and integration** – `PeakForgeR()` performs QC-based
  retention time optimisation, peak boundary refinement, and peak
  integration via Skyline in Docker. Supports multi-method and
...
- **Standalone batch correction** – `batchCorrectR()` provides
  ...
  a simple data.frame and can be used outside the main MStargetR
  pipeline.
- **Interactive Shiny application** – `launchMStargetR()` launches a GUI
  for running the full pipeline without writing code.
- **Transition list validation** – `transition_checkR()` verifies that
  all MRM transitions are unique, and
  `compare_mrm_template_with_guide()` confirms that internal standards
  in the transition list match the concentration guide.
...
M4) are currently unable to run `msConvertR()` or `PeakForgeR()` (with
method set to Skyline) due to Docker image architecture constraints on
ARM processors.

## Running on HPC (Apptainer / Singularity)

Most HPC clusters forbid Docker. `msConvertR()` and `PeakForgeR()` accept
an `enable_HPC = TRUE` argument that swaps Docker for Apptainer
...
With these set you can call `msConvertR()` and `PeakForgeR()` exactly as
on a workstation – no other code changes needed.
...
`shinyWidgets`, `htmltools`). If any are missing, `launchMStargetR()`
will display installation instructions.

## Standalone Batch Correction

`batchCorrectR()` can be used independently of the main pipeline to
apply QC-based signal drift and interbatch correction to any targeted
LC-MS dataset. It accepts a data.frame with columns `sample_name`,
`batch`, `sample_type`, `run_order`, and one or more numeric metabolite
columns:
...
and `ComBat` (empirical Bayes). See `?batchCorrectR` for full parameter
documentation.
...
MStargetR is released under the MIT License. See the [LICENSE](LICENSE)
file for details.

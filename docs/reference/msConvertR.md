...
- manifest:
  ...
  and `plateID`. Use this when plate membership for one-file-per-sample
  formats (e.g. `.d`, `.raw`) comes from an instrument worklist or LIMS
  export rather than the folder layout. When `NULL` (default), plate
  membership is inferred from per-plate subfolders under `raw_data/` (a
  file in `raw_data/<plateID>/` belongs to that plate), falling back to
  the filename for files placed flat in `raw_data/`. Multiple
  single-sample files left flat with no grouping are rejected with
  guidance rather than silently producing one folder per sample. A
  multi-sample `.wiff` is always one plate.
